...
class ResenhaLivekitUrlValidator
  def self.acceptable?(value)
    value.present? && ResenhaLivekitUrlValidator.new.valid_value?(value)
  end
  ...
  def valid_value?(value)
    ...
    @error_key =
      if %w[ws wss].exclude?(uri.scheme.to_s) || uri.host.blank? || uri.userinfo.present?
        "resenha_livekit_url_invalid"
      elsif uri.scheme == "ws" && Rails.env.production?
        "resenha_livekit_url_requires_wss"
      end
    @error_key.nil?
  rescue URI::Error
    @error_key = "resenha_livekit_url_invalid"
    false
  end
