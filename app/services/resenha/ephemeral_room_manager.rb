...
module Resenha
  ...
  class EphemeralRoomManager
    ...
    def self.create!(creator:, name:, members: [], moderators: [], **attrs)
      ...
      DistributedMutex.synchronize("resenha-ephemeral-rooms-#{creator.id}") do
        if Resenha::Room.ephemeral.where(creator_id: creator.id).count >= MAX_LIVE_ROOMS_PER_CREATOR
          raise Discourse::InvalidAccess.new(
                  :resenha_ephemeral_room_limit,
                  nil,
                  custom_message: "resenha.errors.ephemeral_room_limit",
                )
        end
