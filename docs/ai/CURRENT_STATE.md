...
CI note: GitHub Actions is active for the current reconciliation and configuration PRs. Merge decisions still use only the official `Discourse Plugin` result for the exact PR head; missing, stale, skipped, cancelled, pending, neutral, or failed evidence is NOT GREEN.
