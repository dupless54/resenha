...
export function reportMicAcquisitionFailure(error, { modal, toasts }) {
  if (PERMISSION_ERROR_NAMES.includes(error?.name)) {
    ...
    toasts.error({
      duration: 8000,
      data: { message: i18n("resenha.room.mic_unavailable") },
    });
  }
}
...
async function showMicPermissionHelp(modal) {
  ...
  helpOpen = true;
  try {
    await modal.show(ResenhaMicPermissionModal);
  } finally {
    helpOpen = false;
  }
}

const ResenhaMicPermissionModal = <template>
  <DModal
    @closeModal={{@closeModal}}
    @title={{i18n "resenha.mic_permission.title"}}
    class="resenha-mic-permission-modal"
  >
    <:body>
      <p class="resenha-mic-permission-modal__body">
        {{i18n "resenha.mic_permission.body"}}
      </p>
      <p class="resenha-mic-permission-modal__instructions">
        {{i18n "resenha.mic_permission.instructions"}}
      </p>
    </:body>
    <:footer>
      <DButton
        @action={{@closeModal}}
        @label="resenha.mic_permission.ok"
        class="btn-primary resenha-mic-permission-modal__ok"
      />
    </:footer>
  </DModal>
</template>;

export default ResenhaMicPermissionModal;
