...
RSpec.describe ResenhaLivekitUrlValidator do
  ...
  it "rejects URLs that are not ws(s)://, have no host, or embed credentials" do
    [
      ...
      expect(validator.error_message).to eq(
        I18n.t("site_settings.errors.resenha_livekit_url_invalid"),
      )
    end
  end
  ...
  it "requires wss:// in production" do
    ...
    expect(validator.error_message).to eq(
      I18n.t("site_settings.errors.resenha_livekit_url_requires_wss"),
    )
  end
